...
function Portfolio_Met() {
  ...
  const fetchPrices = async () => {
    try {
      ...
      try {
        ...
        setEurUsdRate(fxData.meta.regularMarketPrice);
      } catch (err) {
      ...
      const promises = stocks.map(async (stock) => {
        try {
          const response = await axios.get(`${apiBase}/api/finance/${stock.symbol}`);
          const data = response.data.chart.result[0];
          return {
            ...stock,
            currentPrice: data.meta.regularMarketPrice,
            currency: data.meta.currency,
            totalUSD: data.meta.regularMarketPrice * stock.units
          };
        } catch (err) {
          return { ...stock, currentPrice: null, currency: 'USD', totalUSD: null };
        }
      });
  ...
  const getTotalCurrentUSD = () => portfolio.reduce((sum, s) => sum + (s.totalUSD || 0), 0);
  const getTotalBuyUSD = () => stocks.reduce((sum, s) => sum + (s.buyPriceUSD * s.units), 0);
  const getTotalBuyEUR = () => stocks.reduce((sum, s) => sum + s.buyValueEUR, 0);
  const getTotalCurrentEUR = () => eurUsdRate ? getTotalCurrentUSD() / eurUsdRate : null;
              ...
              <tbody>
                  ...
                  return (
                    <tr key={index}>
                      ...
                      <td className={`etf-price ${stock.currentPrice && stock.currentPrice > stock.buyPriceUSD ? 'price-higher' : 'price-lower'}`}>
                        {stock.currentPrice ? formatNumber(stock.currentPrice) : <span className="error-text">N/A</span>}
                      </td>
                      <td className="etf-sell-total">{formatNumber(stock.buyValueEUR)}</td>
                      ...
                      <td className={diffUSD && diffUSD >= 0 ? 'positive' : 'negative'}>
                        {diffUSD !== null ? (
                          <>
                            ...
                            <div style={{ fontSize: '0.7rem', marginTop: '2px' }}>{diffPercent >= 0 ? '+' : ''}{diffPercent.toFixed(2)}%</div>
                          </>
                        ) : <span className="error-text">N/A</span>}
                      </td>
                      <td className={diffEUR && diffEUR >= 0 ? 'positive' : 'negative'}>
                        {diffEUR !== null ? (
                          <>
                            ...
                            <div style={{ fontSize: '0.7rem', marginTop: '2px' }}>{diffPercentEUR >= 0 ? '+' : ''}{diffPercentEUR.toFixed(2)}%</div>
                          </>
                        ) : <span className="error-text">N/A</span>}
                      </td>
                    </tr>
                ...
                <tr className="total-row" style={{ fontSize: '0.75rem' }}>
                  ...
                  <td>
                    ...
                    <div style={{ fontSize: '0.7rem', color: '#9fa3a8', marginTop: '2px' }}>{formatNumber(getTotalBuyUSD())} USD</div>
                  </td>
                  <td>
                    {getTotalCurrentEUR() && <div>{formatNumber(getTotalCurrentEUR())} EUR</div>}
                    <div style={{ fontSize: '0.7rem', color: '#9fa3a8', marginTop: '2px' }}>{formatNumber(getTotalCurrentUSD())} USD</div>
                  </td>
                  <td className={getTotalCurrentUSD() - getTotalBuyUSD() >= 0 ? 'positive' : 'negative'}>
                    <div>{getTotalCurrentUSD() - getTotalBuyUSD() >= 0 ? '+' : ''}{formatNumber(getTotalCurrentUSD() - getTotalBuyUSD())}</div>
                    <div style={{ fontSize: '0.7rem', marginTop: '2px' }}>{((getTotalCurrentUSD() - getTotalBuyUSD()) / getTotalBuyUSD() * 100).toFixed(2)}%</div>
                  </td>
                  <td className={getTotalCurrentEUR() && getTotalCurrentEUR() - getTotalBuyEUR() >= 0 ? 'positive' : 'negative'}>
                    {getTotalCurrentEUR() ? (
                      <>
                        <div>{getTotalCurrentEUR() - getTotalBuyEUR() >= 0 ? '+' : ''}{formatNumber(getTotalCurrentEUR() - getTotalBuyEUR())}</div>
                        <div style={{ fontSize: '0.7rem', marginTop: '2px' }}>{((getTotalCurrentEUR() - getTotalBuyEUR()) / getTotalBuyEUR() * 100).toFixed(2)}%</div>
                      </>
                    ) : <span className="error-text">N/A</span>}
                  </td>
                </tr>
